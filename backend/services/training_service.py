...
def auto_train_if_ready(university, subject):
    ...
    # 3. Check PYQ count (Threshold: 3 papers)
    count = get_pyq_count(university, subject)
    if count < 3:
        training_logger.info(f"Insufficient data for {university}/{subject}: {count}/3 papers")
        return

    # 4. Trigger background training
    training_logger.info(f"Triggering auto-train for {university}/{subject} (Count: {count})")
...
def _background_train_task(university, subject):
    ...
    try:
        ...
        # ── Step 1: Prep Data ──────────────────────────────────────────
        papers = get_all_pyq_text(university, subject)
        if not papers:
            training_logger.error("No paper text retrieved from Firestore")
            return
        ...
        training_logger.info(f"Loading base T5-small for fine-tuning...")
        model_name = "t5-small"
        ...
        # ── Step 3: Save & Sync (Logic from model_manager) ─────────────
