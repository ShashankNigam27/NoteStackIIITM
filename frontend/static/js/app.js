// NoteStack Core Interaction Library
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Hero Section Paper Plane Animations Trigger
    const triggerSemester = document.getElementById('trigger-semester');
    const planeContainer = document.getElementById('plane-container');

    if (triggerSemester && planeContainer) {
        triggerSemester.addEventListener('click', (e) => {
            e.preventDefault();
            planeContainer.classList.toggle('active');
        });
    }
});
